...
**Technical deep dive:** “The publication selects four tables and the replication slot preserves Debezium's LSN position. Topic names are `mdep.commerce.<table>`. Primary-key routing gives per-key partition order, never global order. The envelope carries before/after/source/op. Deletes have `after=null` and a tombstone is enabled. I configured Debezium 3.0 `provide.transaction.metadata=true`, so transaction metadata and boundary events are expected where supported, but I would only describe their exact shape after runtime inspection. Restarts use Connect offsets, but I treat the system as at least once, so downstream state application must be idempotent.”
