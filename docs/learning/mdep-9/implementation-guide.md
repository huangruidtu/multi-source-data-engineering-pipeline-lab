...
For valid rows inside one incoming batch, a window partitions by the natural key and orders by source business version (`retrieved_at` for rates, `updated_at` for locations), then `source_extract_ts`, `ingested_at`, and `record_hash`, all descending. Rank 1 wins; every lower-ranked row is Quarantined as `duplicate_business_key_non_winner`. The final hash ordering makes an exact timestamp tie deterministic.

The target tables are created in namespace `mdep.silver` using `USING iceberg`; their locations are deterministic below the configured HadoopCatalog warehouse. The SQL `MERGE` uses the natural key and exactly the same lexicographic version order against the existing Silver row. An incoming rate updates only when `(retrieved_at, source_extract_ts, ingested_at, record_hash)` is greater; a location updates only when `(updated_at, source_extract_ts, ingested_at, record_hash)` is greater. `record_hash` is a final deterministic tie-breaker, **not** a freshness signal: an older replay with a different hash cannot overwrite a newer row. An exact tuple/hash replay is a no-op. Quarantine is overwritten by logical-date partition so replays do not multiply evidence for that interval.
